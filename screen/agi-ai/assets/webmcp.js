    ...
    window.WebMcpClient = class WebMcpClient {
        ...
        _createWidget() {
            ...
            trigger.addEventListener('click', (e) => {
                e.stopPropagation();
                this._toggleExpanded(!this.isExpanded);

                // Dispatch event for parent integration
                window.dispatchEvent(new CustomEvent('webmcp-expanded', {
        ...
        _processConnectionToken(encodedToken) {
            try {
                ...
                if (!server) {
                    server = this.currentServer || window.WEBMCP_SERVER || `ws://${window.location.host}`;
                }
        ...
        _registerWithServer(encodedToken) {
            ...
            return new Promise((resolve, reject) => {
                ...
                regSocket.addEventListener('open', (event) => {
                    ...
                    let connectionData = { token: encodedToken };

                    // Only try to atob if it looks like a base64 encoded JSON token
                    if (encodedToken.length > 50 && /^[A-Za-z0-9+/=]+$/.test(encodedToken)) {
                        try {
                            const jsonStr = atob(encodedToken);
                            connectionData = JSON.parse(jsonStr);
                        } catch (e) {
                            console.warn("[WebMCP] Token looked like base64 but failed to parse, using as plain token.");
                        }
                    }
        ...
        registerTool(name, description, schema, executeFn) {
            ...
            // Ensure we have a valid MCP inputSchema
            const inputSchema = (schema && schema.type) ? schema : {
                type: "object",
                properties: {}
            };
...
// Auto-initialize if loaded in a browser environment
